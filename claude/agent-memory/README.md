...
Ce dossier contient une **mémoire custom** utilisée par les subagents personnalisés de ce dotfiles (ex. `tech-watch-scorer`). Chaque sous-dossier correspond à un subagent :
...
Le chemin de `MEMORY.md` doit être explicite dans le prompt du subagent (ex. `claude/agent-memory/<subagent-name>/MEMORY.md`) — le subagent ne le découvre pas par convention implicite.
...
3. **Partagée entre sessions et entre agents** (un subagent peut lire son état passé d'une session à l'autre).
...
Si un subagent génère une mémoire qui grandit indéfiniment (ex. un log d'activité), introduire une stratégie de rotation dans le code du subagent. Chaque subagent fixe son propre seuil de curation adapté à son format (ex. un seuil de lignes au-delà duquel les entrées anciennes sont déplacées vers un fichier d'archive).
