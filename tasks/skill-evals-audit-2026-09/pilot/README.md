...
| `fm-plugin/` | manifest + les 6 `case.yaml` du portage feynman-mentor — §3.2 ; `no-web-lookup` est la version finale (`context.history_file`), `candide-never-fills-gaps` la version recalibrée. **Corpus migré le 2026-09-23** (Phase 2) → `claude/evals/feynman-mentor/`, version vivante ; ces cas restent l'état du pilote |
...
session du run `discovery-french-trigger`, chemins `/tmp` propres à ce run —
régénéré le 2026-09-23 depuis le plugin du repo et versionné au corpus migré,
cf. audit R4), `report.html` (dérivé du JSON).
